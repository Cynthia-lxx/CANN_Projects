...
if __name__ == "__main__":
    ...
    # Golden-implementation self-check matrix. Keep in sync with the shapes
    # used by scripts/gen_data.py (M1 milestone case matrix).
    # (id, B, M, N, K, dtype, transposeX1, transposeX2, x1_range, x2_range)
    cases = [
        ( 0,   1,   64,  64,   64, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 1,   3,   64,  64,  128, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 2,   1,   64,  64,   64, "bfloat16", False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 3,   1,    1,   1,   32, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 4,   1,    2,   3,   40, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 5,   1,  100,  64,   64, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 6,   1,   64, 1000,  64, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 7,   1,    3,   4,   32, "float16",  False, False, [-0.9, -0.1], [-0.9, -0.1]),
        ( 8,   2,    2,   5,   32, "bfloat16", False, False, [-1.0, 1.0], [-1.0, 1.0]),
        ( 9,   1,   64,  64,   40, "float16",  False, False, [-1.0, 1.0], [-1.0, 1.0]),
        # transpose layout smoke (logic + golden path only)
        (10,   1,   8,   8,   32, "float16",   True, False, [-1.0, 1.0], [-1.0, 1.0]),
        (11,   1,   8,   8,   32, "float16",   False, True, [-1.0, 1.0], [-1.0, 1.0]),
        (12,   1,   8,   8,   32, "float16",   True, True, [-1.0, 1.0], [-1.0, 1.0]),
        (13,   1,   8,   8,   32, "bfloat16",  True, True, [-1.0, 1.0], [-1.0, 1.0]),
    ]
    ...
    for cid, B, M, N, K, dtype_name, tx1, tx2, x1_range, x2_range in cases:
        ...
        # 全负输出语义由 _run_known_value_checks() 的确定性样例覆盖（负 x2 → 行 max 为负），
        # 随机 [-1,1] case 不做符号断言。
